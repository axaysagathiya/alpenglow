use {
    crate::{
        ...
        common::{
            conflicting_types, Stake, MAX_ENTRIES_PER_PUBKEY_FOR_NOTARIZE_LITE,
...
impl ConsensusPool {
    ...
    fn insert_certificate(
        ...
        match cert_type {
            CertificateType::NotarizeFallback(slot, block_id) => {
                events.push(VotorEvent::BlockNotarizeFallback((slot, block_id)));
    ...
    pub fn block_has_notar_fallback_or_stronger(&self, block: Block) -> bool {
    ...
    pub fn take_pending_safe_to_notar(&mut self) -> Vec<Block> {
    ...
    pub fn prune_old_state(&mut self, root_slot: Slot) {
        ...
        // Also prune pending_safe_to_notar for old slots
    ...
    pub fn get_highest_finalization_certs(&self) -> Option<ValidatedBlockFinalizationCert> {
...
mod tests {
    ...
    fn add_certificate(
        ...
        match vote {
            ...
            Vote::Finalize(vote) => {
                assert_eq!(pool.highest_finalized_slot().unwrap_or_default(), vote.slot)
            }
    ...
    fn test_add_certificate_with_types(cert_type: CertificateType, vote: Vote) {
        ...
        let (new_finalized_slot, certs_to_send) = pool
            .add_message(&bank, &Pubkey::new_unique(), message.clone(), &mut vec![])
            .unwrap();
        ...
        let (new_finalized_slot, certs_to_send) = pool
            .add_message(&bank, &Pubkey::new_unique(), message, &mut vec![])
            .unwrap();
        assert!(new_finalized_slot.is_none());
        assert_eq!(certs_to_send, []);
